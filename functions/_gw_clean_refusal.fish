function _gw_clean_refusal -a wt branch flags main head -d 'Why $wt must not be touched at all'
    ...
    # Standing in it is not one of them: gwr steps out to the main worktree and
    # removes it, which is what Zsh has always done and what a caller asking for
    # this worktree by name meant.
    set -g _gw_reply ''
