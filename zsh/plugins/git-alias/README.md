...
The short git commands, defined here so the collection stands on its own.
...
That page is the reference for anything not listed above — `gcb`, `grbi`,
`gsta`, `glog`, and the rest. Nothing here conflicts with it: the definitions
...
`gpsup` and `gmom` resolve the branch they need at the moment you run them.
`gmom` asks the repository what its default branch is — `<remote>/HEAD` first,
which git records at clone time from what the server advertised, then `main`,
`trunk`, `master` in that order.
